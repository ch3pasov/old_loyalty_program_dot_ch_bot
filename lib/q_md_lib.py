...
async def queue_money_drop_by_type(type_name: str):
    ...
    return await create_queue_and_cabinet_delta(**q_md_params)


async def queue_money_drop():
    '''Создать очередь-манидроп случайного типа'''
    type_name = choice_queue_md_type(queue_md_params)
    return await queue_money_drop_by_type(type_name, queue_md=queue_md_params)
